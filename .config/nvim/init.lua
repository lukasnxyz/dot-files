...
vim.o.autoread = true -- reload buffer if changed outside of nvim
...
-- Force uniform indentation for selected languages and ignore ftplugin indent logic.
vim.api.nvim_create_autocmd("FileType", {
  pattern = { "python", "rust", "c", "cpp", "zig", "lua", "latex" },
  callback = function()
    vim.opt.expandtab = true
    vim.opt.shiftwidth = 2
    vim.opt.tabstop = 2
...
end, { desc = "Convert Markdown to HTML and open in browser" })
  ...
  },
  {
    ...
    config = function()
      require("nvim-treesitter").setup({
        ensure_installed = { "c", "c++", "lua", "rust", "python", "haskell", "typst", "markdown" },
        auto_install = true,
        highlight = {
          enable = true,
